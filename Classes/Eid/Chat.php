...
class Chat {
	...
	function init( ?FrontendUserAuthentication $user, $charset , $room = false ): void {
		...
		if( $this->env['LLKey'] == "en" || $this->env['LLKey'] == "default" || $this->env['LLKey'] == '') {
			$this->languageService ->includeLLFile("EXT:jvchat/Resources/Private/Language/locallang.xlf");
		} else {
		...
		if ( !$room ) {
            $room = $this->db->getRoom($this->env['room_id']);
        }
		if ( $room ) {
            $this->room = $room ;
        }
        $this->initCommands(($this->room ?? null));
		...
		foreach($lines as $line) {
					...
					if($paramResult === true) {
					    ...
					    if ( $commandFnc ) {
                            ...
                            if(!$data['hidefeedback']) {
                                ...
                                // error
                                if($cmdResult) {
                                    $out .= '<span class="tx-jvchat-error tx-jvchat-error-' . __LINE__ . '">'. sprintf($cmdResult,$parts[0]).   '</span>';
                                }
                            }
                        }
					} else {
                        $out .= '<span class="tx-jvchat-error tx-jvchat-error-' . __LINE__ . '">/'.$command.' '.implode(' ',$parts).': '.$paramResult.'</span>';
                    }
				}
			...
			if(!$found) {
                ...
                if ( $_ENV["TYPO3_CONTEXT"] == "Development" ) {
                    ...
                    $out .= '<span>' . var_export($this->commands, true) . '</span>';
                }
            }
 ...
 function getEntryTextForEmail( $entries, $room=false ) {
	...
	function _makeexpert($params) {
		...
		if($res) {
			...
			return '<span class="tx-jvchat-error tx-jvchat-error-' . __LINE__ . '">ERROR OR NOTHING TO DO</span>';
	...
	function _togglestatus($params) {
		...
		return $res ? 'TOGGLED' : '<span class="tx-jvchat-error tx-jvchat-error-' . __LINE__ . '">ERROR</span>';
	}

	function _toggleroomstatus($params) {
		$res = $this->db->setRoomStatus($this->room, $params[1]);
		return $res ? ('TOGGLED: '.$params[1].'='.$res) : '<span class="tx-jvchat-error tx-jvchat-error-' . __LINE__ . '">ERROR</span>';
	}
