...
class tx_vjchat_chat {
    ...
    function getMessages($lastid) {#
        ...
        $renderer = tx_vjchat_lib::getRenderer($this->settings , "GetMessages" , "html" )  ;
        ...
        foreach($entries as $entry) {
            ...
            if($entryUser) {
                $userType = tx_vjchat_lib::getUserTypeString($this->room, $entryUser) ;
            } else {
                $userType = 'system' ;
            }
...
//			$messages[] = htmlentities($this->room->welcomemessage);
	...
	function getMessagesX($lastid) {
		...
		foreach($entries as $entry) {
			...
			// prepare message that should be sent to client
			$message = '<span id="msg-' . $id . '" class="tx-vjchat-time">'.strftime($timeFormat, $time).'</span><span class="tx-vjchat-user tx-vjchat-userid-'.$entry->feuser.'">'.$username.'</span>&gt;&nbsp;<span class="tx-vjchat-entry">'.$entryText.'</span>';
			...
			$mid = \TYPO3\CMS\Core\Utility\GeneralUtility::shortMD5(($entry->tstamp).($entry->uid));
			$message = '<div id="cid'.$mid.'" class="tx-vjchat-message-style-'.($entry->style).$groupstyles.'">'.$message.'</div>';
					...
					if($paramResult === true) {
					    ...
					    if ( $commandFnc ) {
                            ...
                            if(!$data['hidefeedback']) {
                                $out .= '<span class="tx-vjchat-ok">/'.$command.' '.implode(' ',$parts).'</span>';
                                $out .= '<br><span class="tx-vjchat-ok">'. $cmdResult .'</span>';
                            } else {
                                // error
                                if($cmdResult) {
                        ...
                        $out .= '<span class="tx-vjchat-error">/'.$command.' '.implode(' ',$parts).': '.$paramResult.'</span>';
                    }
				}
