...
class tx_vjchat_db {
	...
	function getRoomsOfUser($userId)
    {
        ...
        /** @var \TYPO3\CMS\Core\Database\Query\QueryBuilder $queryBuilder */
        $queryBuilder = $this->connectionPool->getConnectionForTable('tx_vjchat_room_feusers_mm')->createQueryBuilder();
        $expr = $queryBuilder->expr();
	...
	function createNewRoom($room) {
        ...
        $connection = $this->connectionPool->getConnectionForTable('tx_vjchat_room') ;
        $queryBuilder = $this->connectionPool->getQueryBuilderForTable('tx_vjchat_room') ;

        if ( $queryBuilder->insert('tx_vjchat_room')->values($data)->execute() ) {
            return $connection->lastInsertId('tx_vjchat_room') ;
        } else {
            return 0 ;
        }
	}
	...
	function updateUserInRoom($roomId, $userId, $isSuperuser = false, $enterlabel = '') {
        ...
        if(!$this->isMemberOfRoom($roomId, $userId, false)) {
            ...
            $queryBuilder->insert('tx_vjchat_room_feusers_mm')->values($data)->execute() ;
	...
	function isMemberOfRoom($roomId, $userId, $depend_on_inroom = true) {
        ...
        $userQuery = $queryBuilder->count( '*' )->from('tx_vjchat_room_feusers_mm' )
	...
	function getRoom($uid) {
        ...
        $roomQuery = $queryBuilder->select( '*' )->from('tx_vjchat_room' )
	...
	function putMessage($roomId, $msg, $style = 0, $user = NULL, $hidden = false, $cruser_id = 0, $tofeuserid = 0) {
        ...
        $queryBuilder->insert('tx_vjchat_entry')->values($data)->execute() ;
	...
	function deleteEntry($entryId) {
        ...
        $queryBuilder = $this->connectionPool->getQueryBuilderForTable('tx_vjchat_entry') ;
        $queryBuilder->update('tx_vjchat_entry')
            ->where(
	...
	function deleteEntries( $time) {
		...
		/** @var \TYPO3\CMS\Core\Database\Query\QueryBuilder $queryBuilder */
        $queryBuilder = $this->connectionPool->getQueryBuilderForTable('tx_vjchat_entry') ;
		return $queryBuilder->delete('tx_vjchat_entry')
	...
	function commitMessage($entryId) {
        ...
        $queryBuilder->insert('tx_vjchat_entry')->values($entry)->execute() ;
	...
	function cleanUpUserInRoom($roomId, $idle = 15, $systemMessageOnLeaving = true, $leaveMessage = '%s leaves chat.', $removeSystemMessagsOlderThan = 60) {
        ...
        $queryBuilder = $this->select_mm_query(
            'fe_users.*,tx_vjchat_room_feusers_mm.invisible as invisible, tx_vjchat_room.moderators as moderator , tx_vjchat_room.experts as experts' ,
            'tx_vjchat_room',
            'tx_vjchat_room_feusers_mm',
            'fe_users',
            $roomId ) ;
        ...
        $expr = $queryBuilder->expr();
        $queryBuilder->andWhere($expr->lte('tx_vjchat_room_feusers_mm.tstamp' , ($this->getTime()-$idle) )) ;
	...
	function getOnlineExperts($roomId, $getHidden = false) {
        ...
        return  $this->getUserList($roomId, 'tx_vjchat_room.experts' , true , $getHidden) ;
	}
	...
	function getOnlineModerators($roomId, $getHidden = false) {
        ...
        return $this->getUserList($roomId, 'tx_vjchat_room.moderators' , true , $getHidden) ;
	}
    ...
    function getUserList($roomId, $userType = false , $inroom = true , $getHidden = false , $notuserTypeArray = false ) {
        ...
        $expr = $queryBuilder->expr();
        $queryBuilder->andWhere($expr->lte('tx_vjchat_room_feusers_mm.tstamp' , $this->getTime() )) ;
        ...
        if( $inroom ) {
            $queryBuilder->andWhere($expr->eq('tx_vjchat_room_feusers_mm.in_room' , 1 )) ;
        }
        /** @var \Doctrine\DBAL\Driver\Statement $rows */
        $rows = $queryBuilder->execute() ;
        // $this->debugQuery( $queryBuilder ) ;
